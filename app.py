...
Respond in markdown only for the visible reply. Keep it punchy, persuasive, and playful. Do not ask any follow-up question yourself because the application will append the next question. Put structured data only in the final JSON block."""
            ...
            for token in ai_model.generate_stream(full_prompt, system_instruction=""):
                ...
                parts = re.split(r'```(?:json)?', full_text)
                visible_text = parts[0]
                
                if len(visible_text) > len(emitted_text):
                    new_chunk = visible_text[len(emitted_text):]
                    emitted_text += new_chunk
                    yield f"data: {json.dumps({'token': new_chunk, 'done': False})}\n\n"
...
Respond in markdown only for the visible reply. Keep it punchy, persuasive, and playful. Do not ask any follow-up question yourself because the application will append the next question. Put structured data only in the final JSON block."""
        ...
        for token in ai_model.generate_stream(full_prompt, system_instruction=""):
            ...
            if len(visible_text) > len(emitted_text):
                new_chunk = visible_text[len(emitted_text):]
                emitted_text += new_chunk
                emit('chat_token', {'token': new_chunk, 'done': False, 'session_id': session_id})
...
def parse_ai_response(text):
    try:
        ...
        if json_match:
            ...
            raw_json_match = re.search(r'\n*\{\s*["\'](?:show_image|extracted_data|student_analysis|ready_to_submit)["\']', text, re.DOTALL)
            if raw_json_match:
                clean_text = text[:raw_json_match.start()].strip()
                # Find the matching closing brace
                json_start = raw_json_match.start()
                # try to extract whatever looks like a json string starting from json_start
                try:
                    # just take the rest of the text and let json.loads handle some trailing space or curly braces balancing
                    json_content = text[json_start:]
                    # Simple heuristic: find the last closing brace
